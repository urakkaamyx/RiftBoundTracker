...
public sealed class MatchHub(MatchRoomService rooms, EmulatorAccessService access, DeckService decks, AppDbContext db) : Hub
{
    ...
    public async Task<HubResult> CreateToken(string roomCode, string cardId, string battlefieldOwnerConnectionId)
    {
        var room = rooms.GetRoom(roomCode);
        if (room is null) return new HubResult(false, "Room not found.");
        var (ok, error) = rooms.CreateToken(room, Context.ConnectionId, cardId, battlefieldOwnerConnectionId);
        if (!ok) return new HubResult(false, error);
        await BroadcastAsync(room);
        return new HubResult(true, null);
    }
    ...
    public async Task<HubResult> ToggleUnitReady(string roomCode, string instanceId)
    {
        var room = rooms.GetRoom(roomCode);
        if (room is null) return new HubResult(false, "Room not found.");
        if (!rooms.ToggleUnitReady(room, Context.ConnectionId, instanceId)) return new HubResult(false, "Could not find that unit.");
        await BroadcastAsync(room);
        return new HubResult(true, null);
    }
